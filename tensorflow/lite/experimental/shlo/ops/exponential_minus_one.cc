...
absl::Status Prepare(ExponentialMinusOneOp& op, const Tensor& input,
  ...
  if (!input.IsQuantized() && IsInteger(input.StorageType())) {
    return absl::FailedPreconditionError(
        "stablehlo.exponential_minus_one does not support integer tensor "
        "types.");
  }
  if (input.IsPerAxisQuantized()) {
    return absl::FailedPreconditionError(
        "stablehlo.exponential_minus_one does not support per axis "
        "quantization.");
  }
  if (BaselineType(input.element_type()) !=
      BaselineType(output.element_type())) {
    return absl::FailedPreconditionError(
        "stablehlo.exponential_minus_one constraint (C1) is not satisfied "
        "(incompatible "
        "baseline types).");
  }
