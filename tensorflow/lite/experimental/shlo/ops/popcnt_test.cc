...
template <class T>
struct NonQuantizedPopcntTest : ::testing::Test {};

TYPED_TEST_SUITE(NonQuantizedPopcntTest, NonQuantizedTestTypes, TestParamNames);

TYPED_TEST(NonQuantizedPopcntTest, ArithmeticTensorsWork) {
...
template <class T>
struct QuantizedPopcntTest : ::testing::Test {};

TYPED_TEST_SUITE(QuantizedPopcntTest, QuantizedTestTypes, TestParamNames);

TYPED_TEST(QuantizedPopcntTest, PerTensorWorks) {
  using StorageT = typename TypeParam::StorageT;
  ...
  absl::c_transform(
      input_data, expected_data.begin(), [zero_point, scale](auto v) {
        ...
        const ExpressedT dequantized_res = popcnt_ref(dequantized_input);
...
TYPED_TEST(QuantizedPopcntTest, PerAxisFails) {
  using StorageT = typename TypeParam::StorageT;
  using ExpressedT = typename TypeParam::ExpressedT;
