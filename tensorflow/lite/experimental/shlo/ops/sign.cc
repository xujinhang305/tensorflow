...
absl::Status Prepare(SignOp& op, const Tensor& input, Tensor& output) {
  ...
  if (!input.IsQuantized() && input.StorageType() == DataType::kI1) {
    return absl::FailedPreconditionError(
        "stablehlo.sign does not support boolean tensors.");
  }
  if (input.IsPerAxisQuantized()) {
    return absl::FailedPreconditionError(
        "stablehlo.sign does not support per axis quantization.");
  }
  if (BaselineType(input.element_type()) !=
      BaselineType(output.element_type())) {
    return absl::FailedPreconditionError(
        "stablehlo.sign constraint (C1) is not satisfied (incompatible "
        "baseline types).");
  }
