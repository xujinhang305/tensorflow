...
#define GEN_PASS_DEF_MERGEACTIVATIONTRANSPOSEPASS
...
class RewriteAddWithActivationTranspose : public OpRewritePattern<AddOp> {
 public:
  ...
  LogicalResult match(AddOp op) const override {
    ...
    if (lhs.getType().cast<TensorType>().getRank() != 4) {
      return failure();
    }
...
class MergeActivationTransposePass
    : public impl::MergeActivationTransposePassBase<
          MergeActivationTransposePass> {
 private:
  void runOnOperation() override;
};

void MergeActivationTransposePass::runOnOperation() {
